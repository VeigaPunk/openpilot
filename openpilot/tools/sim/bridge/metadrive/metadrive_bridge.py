...
class MetaDriveBridge(SimulatorBridge):
  ...
  def spawn_world(self, queue: Queue):
    # Half-res on CI software rasterizers; nearest-upscaled to model input size in metadrive_process.
    render_scale = float(os.environ.get("METADRIVE_RENDER_SCALE", "1"))
    rw, rh = max(1, round(W * render_scale)), max(1, round(H * render_scale))
    sensors = {
