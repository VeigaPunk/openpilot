...
class MetaDriveWorld(World):
  def __init__(self, status_q, config, test_duration, test_run, dual_camera=False):
    ...
    # Panda3D/Cocoa cannot safely initialize after fork; use spawn on Darwin and
    # create all IPC objects from that context (including image_lock).
  ...
  def read_sensors(self, state: SimulatorState):
    while self.vehicle_state_recv.poll(0):
      ...
      # macOS steps can be smaller; avoid false vehicle_not_moving in test runs
